...
$i=0;
$result = mysqli_query(GetMyConnection(), "SELECT Property,value FROM Sql973959_3.mdl_metadata where Id_course<>".$course."and Property in ('s_req_skill', 'd_req_skill', 's_acq_skill', 'd_acq_skill')");
$stringa_where='';
			
while ($row = mysqli_fetch_row($result)){
		$stringa_where=$stringa_where."(Sql973959_3.mdl_metadata.Property='".$row[0]."' and Sql973959_3.mdl_metadata.Value='".$row[1]."') or ";
...
while(list($chiave,$valore)=each($array_category))
	...
	if ($valore=='Entrepreneurial Vision') {
				$result = mysqli_query(GetMyConnection(), "SELECT property_value FROM Sql973959_3.mdl_metadata_descr where category='Entrepreneurial Vision' and (property_name='d_req_skill' or property_name='s_req_skill')");
		...
		if ($valore=='Personal Development') {
				$result = mysqli_query(GetMyConnection(), "SELECT property_value FROM Sql973959_3.mdl_metadata_descr where category='Personal Development' and (property_name='d_req_skill' or property_name='s_req_skill')");
							while ($row = mysqli_fetch_row($result)){
							if (in_array($row[0], $array_metadati['d_req_skill']) or in_array($row[0], $array_metadati['s_req_skill'])) {
							echo '<option value="'.$row[0].'" selected="selected">'.$row[0].'</option>';
							}
		...
		if ($valore=='Communication Skills') {
				$result = mysqli_query(GetMyConnection(), "SELECT property_value FROM Sql973959_3.mdl_metadata_descr where category='Communication Skills' and (property_name='d_req_skill' or property_name='s_req_skill')");
							while ($row = mysqli_fetch_row($result)){
							if (in_array($row[0], $array_metadati['d_req_skill']) or in_array($row[0], $array_metadati['s_req_skill'])) {
							echo '<option value="'.$row[0].'" selected="selected">'.$row[0].'</option>';
							}
		...
		if ($valore=='Economic Skills') {
				$result = mysqli_query(GetMyConnection(), "SELECT property_value FROM Sql973959_3.mdl_metadata_descr where category='Economic Skills' and (property_name='d_req_skill' or property_name='s_req_skill')");
							while ($row = mysqli_fetch_row($result)){
							if (in_array($row[0], $array_metadati['d_req_skill']) or in_array($row[0], $array_metadati['s_req_skill'])) {
							echo '<option value="'.$row[0].'" selected="selected">'.$row[0].'</option>';
							}
		...
		if ($valore=='Technical Skills') {
				$result = mysqli_query(GetMyConnection(), "SELECT property_value FROM Sql973959_3.mdl_metadata_descr where category='Technical Skills' and (property_name='d_req_skill' or property_name='s_req_skill')" );
							while ($row = mysqli_fetch_row($result)){
							if (in_array($row[0], $array_metadati['d_req_skill']) or in_array($row[0], $array_metadati['s_req_skill'])) {
							echo '<option value="'.$row[0].'" selected="selected">'.$row[0].'</option>';
							}
		...
		if ($valore=='Abilità Informatiche') {
						$result = mysqli_query(GetMyConnection(), "SELECT property_value FROM Sql973959_3.mdl_metadata_descr where category='Abilità Informatiche' and (property_name='d_req_skill' or property_name='s_req_skill')" );
									while ($row = mysqli_fetch_row($result)){
