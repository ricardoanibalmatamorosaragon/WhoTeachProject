...
/************************************************/
	...
	$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value) VALUES ($id_course, $id, 'category', '".$cat."')";
	$DB->execute($sql);
	...
	$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value) VALUES ($id_course, $id, 'min_age', '".$value."')";
	$DB->execute($sql);
	...
	$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value) VALUES ($id_course, $id, 'max_age', '".$value."')";
	$DB->execute($sql);
/*
        //Keywords
...
*/
             			...
             			if (($pieces2 = explode (",", $temp)) != false) {
                   			for($j = 0; $j < count($pieces2); $j++) {
                    			...
                    			if(!strlen(trim($white_space)) ==0 ) {                       	
                       				$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value, lang) VALUES ($id_course, $id, 'keywords', '".$white_space."', '".$lang."')";
                       			$DB->execute($sql);
                       			}
                    			}
             			}
                    			...
                    			$white_space = trim($lower);
                    			if(!strlen(trim($white_space)) ==0 ) {                       	
                   			$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value) VALUES ($id_course, $id, 'keywords', '".$white_space."')";
                    			$DB->execute($sql);
                    			}
            			}
			}
	...
	for($i = 0; $i < count($index1); $i++) {
		$value = $index1[$i];
		$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value) VALUES ($id_course, $id, 'difficulty', '".$choices[$value]."')";
		$DB->execute($sql);
	}
	...
	for ($j = 0; $j <= $i; $j++) {
		...
		if($current_checkbox == 1) {
			...
			$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value, grade) VALUES ($id_course, $id, 'd_req_skill', '".$choices[$j]."', '".$current_scale."')";
			$DB->execute($sql);
		}
	}
	...
	for ($j = 0; $j <= $i; $j++) {
		...
		if($current_checkbox == 1) {
			...
			$sql="INSERT INTO mdl_metadata(id_course, id_course_sections, property, value, grade) VALUES ($id_course, $id, 'd_acq_skill', '".$choices[$j]."', '".$current_scale."')";
			$DB->execute($sql);
		}
	}
	...
	if($num_rows == 0 && $visible == 1){
		...
		$sql = "INSERT INTO mdl_metadata(Id_course_sections, Id_course, Property, Value) VALUES('".$id."', $id_course, 'status', 'yellow')";
		$DB->execute($sql);
	}
	...
	elseif ($visible == 0) {
		$sql = "INSERT INTO mdl_metadata(Id_course_sections, Id_course, Property, Value) VALUES('".$id."', $id_course, 'status', 'white')";
		$DB->execute($sql);
	}
